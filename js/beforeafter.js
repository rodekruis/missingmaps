...
$(function() {
    ...
    var createLayer = function(layerid) {
        ...
        switch(split[0]) {
            ...
            case 'google':
                return new L.Google(split[1] || 'ROADMAP');
            case 'osm':
                return new L.TileLayer('http://a.tile.openstreetmap.org/{z}/{x}/{y}.png');
            default:
                return L.mapbox.tileLayer(layerid);
        }
    };
    var map = L.mapbox.map('map', null, {
        center: [6.4654, 2.5315],
        minzoom: 14,
        maxzoom: 18,
        zoom: 14
    });
    ...
    $('#navigation').change(function() {
        ...
        $.each(guineaCities, function(index, item) {
            if (place == item.city) {
                var latlng = L.latLng(item.lat, item.lon);
                map.setView(latlng, 14);
            //replacing the pre-tracing city mapbox layer
    ...
    var left = createLayer(layerids[0]).addTo(map);
    var right = createLayer('osm').addTo(map);
